    ...
    return (
        <main className="bg-neutral-50 min-h-screen">
            ...
            >
                ...
                <div className="relative z-20 container max-w-4xl px-6 text-center mt-16">
                    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
                        <span className="inline-block text-gold font-bold text-sm uppercase tracking-[0.3em] mb-6 border border-gold/30 rounded-full px-5 py-2 bg-gold/10 backdrop-blur-sm">
                            {banner.subtitle}
                        </span>
                        ...
                        <p className="text-white/75 text-xl md:text-2xl max-w-2xl mx-auto leading-relaxed mb-12">
                            {banner.description}
                        </p>
                        <button
                    ...
                    <motion.div layout className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        <AnimatePresence mode="popLayout">
                            {filteredJobs.map(job => (
                                <div key={job._id}>
                                    <JobCard job={job} onApply={(j) => {
                                        setActiveForm(prev => prev?._id === j._id ? null : j);
                                        setTimeout(() => {
                                            document.getElementById(`form-${j._id}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
                                        }, 80);
                                    }} />
                                    {/* Inline form expands after the card's column */}
                                    <AnimatePresence>
                                        {activeForm?._id === job._id && (
                                            <div id={`form-${job._id}`} className="mt-4 scroll-mt-24">
                                                <ApplicationForm
                                                    job={activeForm}
            ...
            <section className="py-24 bg-primary relative overflow-hidden">
                ...
                <div className="container relative z-10 px-6 max-w-5xl mx-auto">
                    ...
                    <div className="text-center mt-16">
                        ...
                        <a href="mailto:careers@mppublicschool.edu.in"
                            className="inline-flex items-center gap-2 text-gold font-bold hover:underline underline-offset-2">
                            <Mail className="w-4 h-4" /> careers@mppublicschool.edu.in
                        </a>
                    </div>
                </div>
            </section>
